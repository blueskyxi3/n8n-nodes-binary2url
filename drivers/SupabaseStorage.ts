...
export class SupabaseStorage {
  ...
  async uploadStream(
    ...
    try {
      ...
      if (buffer.length > 10 * 1024 * 1024) {
        console.warn(
          `Large file (${(buffer.length / 1024 / 1024).toFixed(2)}MB) being uploaded to Supabase. Consider using S3 for better streaming support.`
        );
      }
      ...
      if (error) {
        ...
        if (error.message.includes('Permission denied')) {
          throw new Error(
            `Access denied to Supabase bucket "${this.bucket}". Check your API key and bucket permissions`
          );
        }
  ...
  async downloadStream(fileKey: string): Promise<DownloadResult> {
    try {
      ...
      const contentType =
        response.headers.get('content-type') || this.getContentTypeFromKey(fileKey);
